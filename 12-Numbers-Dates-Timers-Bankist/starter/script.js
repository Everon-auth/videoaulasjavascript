...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    const day = new Date().getDay().toFixed(0,2)
    const month = new Date().getMonth().toFixed(0,2)
    const year = new Date().getFullYear()
      currentAccount.movementsDates.push(`${day}/${month}/${year}`)
    // Update UI
...
const startLogOutTimer = () =>{
  ...
  tick()
  const timing = setInterval(() => timer, 100);
  console.log(timing)
  return timing
}
let timer